...
ROI_RUN_DIR="${1:-output/runs/dinov3_vitb_hidden_context_roi_anchor_hcivcplax_v2_seed42}"
SUBMISSION_DIR="${2:-output/submissions/dinov3_vitb_hidden_context_roi_anchor_hcivcplax_v2_seed42}"
...
python scripts/roi_refine_submission_from_anchor.py \
  ...
  --roi-context 1.80 \
  --roi-min-size 112 \
  --roi-gate-task-thresholds HC=5,IVC=5,PLAX=5 \
  --batch-size 8 && \
