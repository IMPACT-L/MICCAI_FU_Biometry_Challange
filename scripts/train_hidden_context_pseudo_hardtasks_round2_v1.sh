...
OUTPUT_DIR="${1:-output/runs/dinov3_vitb_taskfpn_hidden_context_pseudo_hardtasks_round2_v1_seed42}"
BASE_CKPT="${2:-output/runs/dinov3_vitb_taskfpn_hidden_context_pseudo_hardtasks_v1_seed42/checkpoints/best_model.pth}"
BASE_SUBMISSION_JSON="${3:-output/submissions/dinov3_vitb_taskfpn_hidden_context_pseudo_hardtasks_v1_seed42/regression_predictions.json}"
PSEUDO_ROOT="${4:-output/pseudo_datasets/dinov3_vitb_taskfpn_hidden_context_pseudo_hardtasks_round2_v1_seed42}"
SEED="${5:-42}"

python scripts/build_pseudo_dataset_from_submission.py \
  --base-data-root data \
  --manifest data/manifests/validation_manifest.csv \
  --submission-json "${BASE_SUBMISSION_JSON}" \
...
python baseline/train.py \
  ...
  --measurement-task-weights FA=1.00,HC=1.30,IVC=1.70,PLAX=0.90 \
  --dataset-loss-weight 0.0 \
  --femur-shaft-loss-weight 0.0 \
  --fugc-segment-loss-weight 0.0 \
  --ivc-band-loss-weight 0.10 \
  --task-loss-weights A4C=1.15,FA=1.15,HC=1.40,IVC=1.55,PLAX=1.20 \
  --sampler-task-weights A4C=1.10,FA=1.10,HC=1.40,IVC=1.55,PLAX=1.20 \
  --ema-decay 0.999 \
