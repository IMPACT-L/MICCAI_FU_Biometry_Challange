...
OUTPUT_DIR="${1:-output/runs/dinov3_vitb_hidden_context_roi_anchor_hcivcplax_v2_seed42}"
...
python baseline/train.py \
  ...
  --roi-context-min 1.55 \
  --roi-context-max 2.05 \
  --roi-center-jitter 0.04 \
  --anchor-consistency-weight 0.02 \
  --anchor-consistency-tasks HC,IVC,PLAX \
  --measurement-loss-weight 0.006 \
  --measurement-loss-tasks HC,IVC,PLAX \
  --measurement-task-weights HC=1.25,IVC=1.50,PLAX=1.00 \
  --ivc-band-loss-weight 0.08 \
  --checkpoint-task-weights HC=1.50,IVC=1.80,PLAX=1.35 \
