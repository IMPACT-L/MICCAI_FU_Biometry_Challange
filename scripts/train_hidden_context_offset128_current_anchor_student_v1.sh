...
OUTPUT_DIR="${1:-output/runs/dinov3_vitb_hidden_context_offset128_current_anchor_student_v1_seed42}"
BASE_CKPT="${2:-output/runs/dinov3_vitb_hidden_context_offset128_v1_seed42/checkpoints/best_model.pth}"
TEACHER_JSON="${3:-output/submissions/dinov3_vitb_hidden_context_roi_hcivcplax_v1_seed42/regression_predictions.json}"
PSEUDO_ROOT="${4:-output/pseudo_datasets/dinov3_vitb_hidden_context_offset128_current_anchor_student_v1}"
SEED="${5:-42}"
...
python baseline/train.py \
  ...
  --aop-angle-loss-weight 0.0 \
  --task-loss-weights A4C=1.10,AOP=1.05,FA=1.00,FUGC=1.00,HC=1.25,IVC=1.30,PLAX=1.15,PSAX=1.00,fetal_femur=1.00 \
  --sampler-task-weights A4C=1.10,HC=1.25,IVC=1.30,PLAX=1.15 \
  --ema-decay 0.999 \
